...
double ref_v = 80*.45; // 80 mph in m/s
...
// Previously working tuned parameters, but max speed is only 40mph   
...
// ////////////////////
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      fg[0] += CppAD::pow(vars[a_start + t], 2);
      fg[0] += 3*CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2);
    ...
    for (int t = 1; t < N; t++) {
      ...
      // desired heading at a given x is the angle at the derivative
